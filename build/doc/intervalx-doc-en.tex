...
\begin{function}{scaled}
  ...
  \end{syntax}
  It is also possible to adjust the brackets/parentheses' size through the \texttt{scaled} key, which has the same usage as in the \pkg{interval} package: \texttt{auto} applies \cs{left} and \cs{right}, while the other options use the delimiters \cs{bigl}/\cs{bigr}\dots
\end{function}
